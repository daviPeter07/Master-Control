<div id="edit-client-modal" class="fixed inset-0 bg-black/60 hidden items-center justify-center z-50">
  <div class="bg-[var(--color-surface)] p-6 rounded-lg shadow-xl w-full max-w-md m-4">
    ...
    <form action="/src/actions/clientes/edit_cliente_action.php" method="POST">
      <input type="hidden" name="id" id="edit-id">

      <div class="space-y-4">
        <div>
          <label for="edit-nome" class="block text-sm font-medium text-[var(--color-text-secondary)] mb-1">Nome Completo</label>
          <input type="text" name="nome" id="edit-nome" required placeholder="Nome completo" class="name-input w-full p-2 border border-[var(--color-border)] rounded-lg bg-[var(--color-background)] text-[var(--color-text-primary)] focus:outline-none focus:ring-1 focus:ring-[var(--color-primary)]">
        </div>
        <div>
        ...
        <div>
          ...
          <input type="tel" name="telefone" id="edit-telefone" placeholder="Telefone (opcional)" class="phone-input w-full p-2 border border-[var(--color-border)] rounded-lg bg-[var(--color-background)] text-[var(--color-text-primary)] focus:outline-none focus:ring-1 focus:ring-[var(--color-primary)]">
        </div>
